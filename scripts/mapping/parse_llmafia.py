...
def parse_records(raw_dir: Path) -> List[Dict]:
    ...
    for file in files:
        ...
        for row in rows:
            text = str(row.get("text") or row.get("message") or row.get("content") or "").strip()
            if not text and row.get("event_type") == "vote":
            ...
            out.append(
                {
                    "source": SOURCE,
                    "game_id": str(row.get("game_id") or row.get("game") or file.stem),
                    "phase": phase,
                    "day_index": day_index,
                    "turn_index": turn,
                    "speaker": str(row.get("speaker") or row.get("sender") or row.get("player") or "unknown"),
                    ...
                    "raw_metadata": {
                        ...
                        "timing_delta_prev_msg": row.get("timing_delta_prev_msg"),
                        "timing_delta_prev_self": row.get("timing_delta_prev_self"),
                        "raw_row": row,
                    },
                }
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Parse llmafia raw data into intermediate events.")
