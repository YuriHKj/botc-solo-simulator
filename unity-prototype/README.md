...
当前产品能力等级、promotion gates 与公开分发边界只在 `../docs/CAPABILITY_STATUS.md` 维护。默认玩家路径使用 Unity、Trouble Brewing 与确定性 AI；本页只说明 Unity 的运行和构建方式。
...
AI 发言润色是仅供开发评估的可选实验功能，不属于默认玩家路径。构建版如果在 exe 同级看到 `botc_ai_polish.enabled`，或用 `-botc-llm-renderer` / `-botc-ai-polish` 启动，会优先查找 exe 同级的 `LocalLLM/llama-server.exe` 与 `LocalLLM/models/*.gguf`，自动拉起本地 OpenAI-compatible 服务；如果缺失，会回退到外部 endpoint 或 Ollama，再失败则使用 JS Core 的确定性文本。开发打包说明见 `docs/packaging/AI_POLISHED_UNITY_RELEASE.md`。
