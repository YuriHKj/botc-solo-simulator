...
function deliverSpyPeek(ctx, spy) {
  ...
  const reveal = ctx.shuffle(ctx.state.players)
    ...
    .join(" | ");
  const text = `[第${ctx.state.night}夜] 你看到了魔典：${reveal}`;
  ctx.addPrivateInfo(ctx.state, spy, text);
  ctx.state.events.infoPings.push({ night: ctx.state.night, actorId: spy.id, type: "spy", text });
}
