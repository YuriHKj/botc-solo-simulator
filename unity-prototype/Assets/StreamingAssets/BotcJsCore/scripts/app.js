...
import { configureAudio, playCue, primeAudio, setMusicMood, stopBackgroundMusic } from "./audio.js";
import { initUI, promptNightActionChoice, renderGame, showPrivateDialogue, showToast } from "./ui.js";
import { buildUnityViewModel } from "./unity_viewmodel.js";
...
let lastUnityViewModelSyncAt = 0;
...
function refresh() {
  ...
  syncUnityViewModel(insights);
  syncAudioScene();
  persistLatestSave();
}

function syncUnityViewModel(aiInsights) {
  if (!window.botcBridge?.writeUnityViewModel) {
    return;
  }
  const now = Date.now();
  if (now - lastUnityViewModelSyncAt < 300) {
    return;
  }
  lastUnityViewModelSyncAt = now;
  try {
    const viewModel = buildUnityViewModel(state, { aiInsights });
    window.botcBridge.writeUnityViewModel(viewModel).catch((error) => {
      console.warn("Unity viewmodel export failed", error);
    });
  } catch (error) {
    console.warn("Unity viewmodel build failed", error);
  }
}
...
function runProactiveAIWhispersIfReady({ showFirst = false } = {}) {
  ...
  if (messages.length > 0) {
    toastParts.push(`${messages.length} 名玩家给你递了私聊信息`);
  }
  if (peerMessages.length > 0) {
    toastParts.push(`${peerMessages.length} 组 AI 进行了私聊`);
  }
  showToast(`${toastParts.join("，")}，已记入事件日志；需要时再点玩家继续聊。`);
  refresh();
  return messages;
}
...
function handleAfterNightResolution() {
  const hasPendingActions = handlePendingStorytellerActions({
    onDrained: () => runProactiveAIWhispersIfReady({ showFirst: false }),
  });
  if (!hasPendingActions) {
    runProactiveAIWhispersIfReady({ showFirst: false });
  }
}
...
function loadSettings() {
  try {
    ...
    settingsState = {
      ...
      musicVolume: clampNum(parsed?.musicVolume, 0, 100, DEFAULT_SETTINGS.musicVolume),
    };
  } catch {
    settingsState = { ...DEFAULT_SETTINGS };
  }
...
function applySettings() {
  ...
  document.documentElement.style.setProperty("--music-volume", `${settingsState.musicVolume / 100}`);
  configureAudio({
    masterVolume: settingsState.masterVolume / 100,
    sfxVolume: settingsState.sfxVolume / 100,
    musicVolume: settingsState.musicVolume / 100,
  });
...
function syncSettingsControls() {
  ...
  settingsDom.musicVolume.value = `${settingsState.musicVolume}`;
  ...
  settingsDom.musicVolumeValue.textContent = `${Math.round(settingsState.musicVolume)}%`;
}
...
function saveSettingsFromControls() {
  settingsState = {
    ...
    musicVolume: clampNum(settingsDom.musicVolume.value, 0, 100, DEFAULT_SETTINGS.musicVolume),
  };
  persistSettings();
  applySettings();
  syncSettingsControls();
...
function doWhisper({ targetId, message, intentHint = "generic", deception = {} }) {
  try {
    ensureState();
    const result = runPrivateWhisper(state, { targetId, humanLine: message, intentHint, deception }, rng);
    if (!result.ok) {
...
function pushHumanPublicTimeline(roleName) {
  ...
  const record = {
    ...
    speakerId: human?.id ?? "",
    targetId: null,
    text: `我公开声称自己是 ${roleName}。`,
    roundInDay: state.dayStageMeta?.publicRounds ?? 0,
    orderIndex: Number.isFinite(state.aiDialogue.timeline.length) ? state.aiDialogue.timeline.length : 0,
    payload: { humanClaim: true, roleName },
  };
...
function publicClaim({ roleId }) {
  try {
    ensureState();
    if (state.phase !== "day" || state.gameOver || !["public", "nomination"].includes(state.dayStage)) {
      showToast("对外声称身份建议在公聊或提名阶段使用。");
      refresh();
      return;
    }
    const human = state.players.find((entry) => entry.isHuman);
    ...
    if (!human || !role) {
      showToast("请选择一个要公开声称的身份。");
      refresh();
      return;
    }
    registerClaim(state, human.id, role.id);
    pushHumanPublicTimeline(role.name);
    showToast(`你公开声称自己是 ${role.name}。AI 会把它当作公开证词记录。`);
    playCue("public");
    refresh();
  } catch (error) {
    const text = error instanceof Error ? error.message : "公开声称身份失败。";
    showToast(text);
  }
}
...
function executeNomination({ nominatorId, nomineeId, humanVoteYes }) {
  try {
    ...
    if (!result.passed && !state.gameOver) {
      showToast(`投票未通过：${result.yesVotes}/${result.threshold}。今天仍可继续提名。`);
      playCue("phase");
      refresh();
      return;
    }
...
function bindSettingsEvents() {
  ...
  settingsDom.musicVolume = document.getElementById("settingMusicVolumeRange");
  ...
  settingsDom.musicVolumeValue = document.getElementById("settingMusicVolumeValue");
  ...
  settingsDom.musicVolume.addEventListener("input", () => {
    settingsDom.musicVolumeValue.textContent = `${settingsDom.musicVolume.value}%`;
  });
...
function boot() {
  ...
  initUI({
    ...
    onProcessStorytellerQueue: () => handlePendingStorytellerActions(),
  });
  ...
  document.addEventListener(
    ...
    (event) => {
      primeAudio();
      if (event.target instanceof Element && event.target.closest("button, .player-token, select")) {
        playCue("click");
      }
    },
    { capture: true }
