...
const PORTRAIT_ASSETS = [
  "./assets/generated/portraits/villager_01.svg",
  "./assets/generated/portraits/villager_02.svg",
  "./assets/generated/portraits/villager_03.svg",
  "./assets/generated/portraits/villager_04.svg",
  "./assets/generated/portraits/villager_05.svg",
];
...
const seenVoteCeremonyIds = new Set();
...
function makePlayerToken(
  ...
  hint.className = "token-action-hint";
  hint.textContent = whisperable ? "左键私聊 · 右键标记" : "点击标记 · 右键魔典";
...
function debateModeLabel(item) {
  if (item.mode === "public") {
    return item.debateBeatLabel || "公聊";
  }
...
function renderDebateStage(state) {
  ...
  timeline.forEach((item, idx) => {
    ...
    if (item.targetId) {
      target.textContent = `${item.debateBeatLabel ? `${item.debateBeatLabel} · ` : ""}-> ${playerNameById(state, item.targetId)}`;
    } else {
...
function compactTimelineLabel(item, state) {
  if (!item) {
    return "";
  }
  const speaker = playerNameById(state, item.speakerId);
  const target = item.targetId ? ` -> ${playerNameById(state, item.targetId)}` : "";
  if (item.mode === "public") {
    const order = Number.isFinite(item.orderIndex) ? `#${item.orderIndex + 1}` : "公聊";
    const beat = item.debateBeatLabel ? ` · ${item.debateBeatLabel}` : "";
    return `${order}${beat} ${speaker}${target}`;
  }
  if (item.mode === "whisper-out") {
    return `你问 ${playerNameById(state, item.targetId)}`;
  }
  return `${speaker}${target}`;
}

function renderTimelineRail(state) {
  if (!dom.timelineRail || !dom.timelineRailList || !dom.timelineRailMeta) {
    return;
  }
  const timeline = (state.aiDialogue?.timeline ?? []).slice(-6);
  dom.timelineRailList.innerHTML = "";
  if (timeline.length === 0) {
    dom.timelineRailMeta.textContent = "暂无";
    const empty = document.createElement("div");
    empty.className = "timeline-rail-empty";
    empty.textContent = "私聊、公聊和公开声称会显示在这里。";
    dom.timelineRailList.appendChild(empty);
    return;
  }
  const latest = timeline[timeline.length - 1];
  dom.timelineRailMeta.textContent = `D${latest.day ?? state.day} · ${debateModeLabel(latest)}`;
  timeline.forEach((item) => {
    const row = document.createElement("article");
    row.className = `timeline-rail-item ${item.mode ?? "event"}`;
    const title = document.createElement("strong");
    title.textContent = compactTimelineLabel(item, state);
    const text = document.createElement("span");
    text.textContent = `${item.text ?? ""}`.slice(0, 72);
    row.appendChild(title);
    row.appendChild(text);
    dom.timelineRailList.appendChild(row);
  });
}

function latestVoteEvent(state) {
  const animation = state.lastVoteAnimation;
  if (animation?.day === state.day) {
    return { ...animation, animated: true };
  }
  const votes = state.events?.votes ?? [];
  if (votes.length === 0) {
    return null;
  }
  const latest = votes[votes.length - 1];
  return latest?.day === state.day ? latest : null;
}

function renderVoteCeremony(state) {
  if (!dom.voteCeremony || !dom.voteCeremonyTitle || !dom.voteCeremonyCount || !dom.voteCeremonyVoters) {
    return;
  }
  const vote = latestVoteEvent(state);
  if (!vote || state.phase !== "day") {
    dom.voteCeremony.classList.add("hidden");
    return;
  }
  const voteId = `${vote.day}:${vote.nominatorId}:${vote.nomineeId}:${state.events.votes.length}`;
  dom.voteCeremony.classList.remove("hidden", "passed", "failed", "just-voted", "voting-sequence");
  dom.voteCeremony.classList.add(vote.passed ? "passed" : "failed");
  if (vote.animated) {
    dom.voteCeremony.classList.add("voting-sequence");
  }
  if (!seenVoteCeremonyIds.has(voteId)) {
    dom.voteCeremony.classList.add("just-voted");
    seenVoteCeremonyIds.add(voteId);
  }
  const nominee = playerNameById(state, vote.nomineeId);
  const nominator = playerNameById(state, vote.nominatorId);
  dom.voteCeremonyTitle.textContent = vote.animated
    ? `${nominator} 提名 ${nominee}：依次举手`
    : `${nominee} 的投票${vote.passed ? "通过" : "未通过"}`;
  dom.voteCeremonyCount.textContent = `${vote.yesVotes} / ${vote.threshold}`;
  dom.voteCeremonyVoters.innerHTML = "";
  (vote.votes ?? []).forEach((entry, index) => {
    const voter = playerNameById(state, entry.voterId);
    const pill = document.createElement("span");
    pill.className = `vote-voter-pill ${entry.vote ? "yes" : entry.abstain ? "abstain" : "no"}`;
    pill.style.animationDelay = `${index * 90}ms`;
    pill.textContent = `${voter}${entry.vote ? " ✓" : entry.abstain ? " ·" : " ×"}`;
    dom.voteCeremonyVoters.appendChild(pill);
  });
}
...
function renderPublicClaimOptions(state) {
  if (!dom.publicClaimRole) {
    return;
  }
  const roles = getAllRoles(state.scriptId).map((role) => ({
    id: role.id,
    label: `[${role.categoryName ?? role.category}] ${role.name}`,
  }));
  const human = state.players.find((entry) => entry.isHuman);
  const selected = dom.publicClaimRole.value || human?.publicClaimRoleId || human?.roleId || roles[0]?.id || "";
  fillSelect(dom.publicClaimRole, roles, selected, "选择声称身份");
}

function renderPrivateDeceptionOptions(state) {
  if (!dom.chatDeceptionRole) {
    return;
  }
  const roleOptions = getAllRoles(state.scriptId).map((role) => ({
    id: role.id,
    label: `[${role.categoryName ?? role.category}] ${role.name}`,
  }));
  fillSelect(dom.chatDeceptionRole, roleOptions, dom.chatDeceptionRole.value || "", "不私下声称");
}
...
function triggerWhisper(handlers, options = {}) {
  ...
  const deception =
    options.deception ??
    {
      claimRoleId: dom.chatDeceptionRole?.value ?? "",
      nightInfo: `${dom.chatDeceptionNightInput?.value ?? ""}`.trim(),
      askSecret: !!dom.chatDeceptionSecret?.checked,
    };
  handlers.onWhisper({
    targetId,
...
function renderHud(state) {
  ...
  if (dom.officialScriptLine && dom.officialSetupLine && dom.officialStateLine) {
    ...
    dom.officialScriptLine.textContent = scriptCoreName;
...
function phaseObjectiveFor(state) {
  const queueCount = Array.isArray(state.pendingStorytellerActions) ? state.pendingStorytellerActions.length : 0;
  if (queueCount > 0) {
    return {
      title: `处理 Storyteller 队列（${queueCount}）`,
      hint: "流程已暂停。先处理死亡触发、贤者信息、理发师交换等即时选择。",
    };
  }
  const nightAction = getHumanNightActionState(state);
  const dayAction = getHumanDayActionState(state);
  if (state.gameOver) {
    return { title: "复盘本局", hint: "查看事件日志、AI 证据簿和关键死亡触发，然后可以回到主菜单新开一局。" };
  }
  if (state.phase === "night") {
    return nightAction.available
      ? { title: `夜间行动：${nightAction.roleName}`, hint: "Storyteller 正在等待你的选择。也可以让系统自动代选。" }
      : { title: "夜晚结算中", hint: "系统会按夜间顺序处理技能、死亡和私有信息。" };
  }
  if (state.dayStage === "private") {
    return { title: "私聊收集线索", hint: "点击其他玩家 token 可以直接私聊；右键 token 或按住 Ctrl/Shift 点击可做魔典标记。" };
  }
  if (state.dayStage === "public") {
    return { title: "公聊形成公开信息", hint: "进行公聊后观察 token 高亮和对话时间轴，再决定是否进入提名。" };
  }
  if (dayAction.available) {
    return { title: `可发动：${dayAction.roleName}`, hint: "可以点击行动按钮，也可以先提名投票。白天技能会以 Storyteller 弹窗处理。" };
  }
  return { title: "提名与投票", hint: "选择提名者、被提名者和你的投票；也可以让 AI 发起压力提名。" };
}

function renderPhaseObjective(state) {
  if (!dom.phaseObjectiveHud) {
    return;
  }
  const objective = phaseObjectiveFor(state);
  dom.phaseObjectiveTitle.textContent = objective.title;
  dom.phaseObjectiveHint.textContent = objective.hint;
}

function renderStorytellerQueue(state) {
  if (!dom.storytellerQueueHud || !dom.storytellerQueueList || !dom.storytellerQueueCount) {
    return;
  }
  const queue = Array.isArray(state.pendingStorytellerActions) ? state.pendingStorytellerActions : [];
  dom.storytellerQueueHud.classList.toggle("hidden", queue.length === 0);
  dom.storytellerQueueCount.textContent = `${queue.length}`;
  dom.storytellerQueueList.innerHTML = "";
  queue.slice(0, 3).forEach((entry, idx) => {
    const item = document.createElement("div");
    item.className = "storyteller-queue-item";
    const role = entry.roleName ?? "Storyteller";
    const phase = entry.phaseLabel ?? `D${entry.createdDay ?? "-"} / N${entry.createdNight ?? "-"}`;
    item.innerHTML = `<strong>${idx + 1}. ${role}</strong><span>${phase} · ${compactText(entry.prompt ?? entry.type ?? "等待选择", 34)}</span>`;
    dom.storytellerQueueList.appendChild(item);
  });
  if (queue.length > 3) {
    const more = document.createElement("div");
    more.className = "storyteller-queue-more";
    more.textContent = `还有 ${queue.length - 3} 项等待处理`;
    dom.storytellerQueueList.appendChild(more);
  }
  if (dom.btnProcessStorytellerQueue) {
    dom.btnProcessStorytellerQueue.disabled = queue.length === 0;
  }
}
...
function updateChatDramaComposerState(canWhisper = false) {
  ...
  if (dom.chatDeceptionRole) {
    dom.chatDeceptionRole.disabled = !enabled;
  }
  if (dom.chatDeceptionNightInput) {
    dom.chatDeceptionNightInput.disabled = !enabled;
  }
  if (dom.chatDeceptionSecret) {
    dom.chatDeceptionSecret.disabled = !enabled;
  }
  if (dom.chatDramaQuickGrid) {
...
function openChatDramaModal(payload) {
  ...
  if (dom.chatDeceptionRole) {
    dom.chatDeceptionRole.value = "";
  }
  if (dom.chatDeceptionNightInput) {
    dom.chatDeceptionNightInput.value = "";
  }
  if (dom.chatDeceptionSecret) {
    dom.chatDeceptionSecret.checked = false;
  }
...
function setButtonState(state) {
  ...
  if (dom.btnPublicClaim) {
    dom.btnPublicClaim.disabled = !(inDay && ["public", "nomination"].includes(stage) && !disabled);
  }
  if (dom.publicClaimRole) {
    dom.publicClaimRole.disabled = !(inDay && ["public", "nomination"].includes(stage) && !disabled);
  }
...
export function initUI(handlers) {
  ...
  dom.publicClaimRole = qs("publicClaimRoleSelect");
  dom.btnPublicClaim = qs("btnPublicClaim");
  ...
  dom.phaseObjectiveHud = qs("phaseObjectiveHud");
  dom.phaseObjectiveTitle = qs("phaseObjectiveTitle");
  dom.phaseObjectiveHint = qs("phaseObjectiveHint");
  dom.storytellerQueueHud = qs("storytellerQueueHud");
  dom.storytellerQueueCount = qs("storytellerQueueCount");
  dom.storytellerQueueList = qs("storytellerQueueList");
  dom.btnProcessStorytellerQueue = qs("btnProcessStorytellerQueue");
  dom.timelineRail = qs("timelineRail");
  dom.timelineRailMeta = qs("timelineRailMeta");
  dom.timelineRailList = qs("timelineRailList");
  dom.voteCeremony = qs("voteCeremony");
  dom.voteCeremonyTitle = qs("voteCeremonyTitle");
  dom.voteCeremonyCount = qs("voteCeremonyCount");
  dom.voteCeremonyVoters = qs("voteCeremonyVoters");
  ...
  dom.chatDeceptionRole = qs("chatDeceptionRoleSelect");
  dom.chatDeceptionNightInput = qs("chatDeceptionNightInput");
  dom.chatDeceptionSecret = qs("chatDeceptionSecretCheck");
  ...
  dom.btnProcessStorytellerQueue?.addEventListener("click", () => {
    uiHandlers?.onProcessStorytellerQueue?.();
  });
  ...
  dom.btnPublicClaim?.addEventListener("click", () => {
    handlers.onPublicClaim?.({ roleId: dom.publicClaimRole?.value ?? "" });
  });
...
export function renderGame(state, aiInsights) {
  ...
  renderTimelineRail(state);
  renderVoteCeremony(state);
  renderNominationOptions(state);
  renderPublicClaimOptions(state);
  renderPrivateDeceptionOptions(state);
  ...
  renderBluffs(state);
  renderGuide(state);
  renderPhaseObjective(state);
  renderStorytellerQueue(state);
